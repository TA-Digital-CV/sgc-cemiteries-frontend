...
export function useAnalytics(): UseAnalyticsReturn {
  ...
  const fetchMetrics = useCallback(async (cemeteryId?: string) => {
    try {
      ...
      const occJson: unknown = await occRes.json();
      const overall = (occJson as any)?.overall ?? {};
  ...
  const fetchOccupancyData = useCallback(async (cemeteryId?: string) => {
    try {
      ...
      const overall = (json as any)?.overall ?? {};
      const data: OccupancyData[] = [
        {
          date: new Date().toISOString().split("T")[0],
          cemeteryId: String(cemeteryId),
          cemeteryName: String((json as any)?.cemeteryName ?? ""),
  ...
  const fetchStatistics = useCallback(
    async (customFilters?: AnalyticsFilters) => {
      try {
        ...
        if (!res.ok) throw new Error(`Fetch failed: ${res.status}`);
        const json: unknown = await res.json();
        const occ = (json as any)?.occupancyStats ?? {};
        const dist = (json as any)?.plotTypeDistribution ?? {};
        const item: CemeteryStatistics = {
          cemeteryId: String((json as any)?.cemeteryId ?? cemeteryId),
          cemeteryName: String((json as any)?.cemeteryName ?? ""),
          ...
          capacityProjection: {
            ...
            projectionDate:
              (json as any)?.capacityProjection?.estimatedFullCapacityDate ??
              "",
            monthsRemaining: Number(
              (json as any)?.capacityProjection?.monthsRemaining ?? 0,
            ),
  ...
  const fetchProjections = useCallback(
    async (cemeteryId?: string, years: number = 12) => {
      try {
        ...
        if (!res.ok) throw new Error(`Fetch failed: ${res.status}`);
        const json: unknown = await res.json();
        const p = (json as any)?.projections ?? {};
        const params = (json as any)?.projectionParameters ?? {};
        const current = (json as any)?.currentStatus ?? {};
        const item: CapacityProjection = {
          ...
          scenario: {
            type: "MODERATE",
            growthRate: Number(
              (json as any)?.historicalTrends?.growthRate ?? 0,
            ),
            description: "",
            assumptions: [],
          },
  ...
  const fetchHeatmapData = useCallback(async (cemeteryId?: string) => {
    try {
      ...
      if (!res.ok) throw new Error(`Fetch failed: ${res.status}`);
      const json: unknown = await res.json();
      const grid: unknown[] = Array.isArray((json as any)?.data)
        ? (json as any).data
        : [];
      const items: HeatmapData[] = grid.map((cell, idx) => ({
        ...
        cemeteryName: String((json as any)?.cemeteryName ?? ""),
        geoPoint: {
          latitude: Number((cell as any)?.coordinates?.latitude ?? 0),
          longitude: Number((cell as any)?.coordinates?.longitude ?? 0),
        },
        metric: "occupancy",
        value: Number((cell as any)?.value ?? 0),
        intensity: Number((cell as any)?.value ?? 0),
        label: `${Number((cell as any)?.value ?? 0).toFixed(2)}`,
      }));
  ...
  const exportData = useCallback(
    async (_format: "csv" | "json" | "pdf"): Promise<ActionResult<unknown>> => {
      try {
        setLoading(true);
        setError(null);
