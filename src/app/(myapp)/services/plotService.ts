...
export class PlotService {
  ...
  async getPlotQRCode(id: string): Promise<{ plotId: string; qrCode: string }> {
    const url = `${this.baseUrl}/plots/${id}/qr-code`;
    const response: any = await this.fetchWithErrorHandling<any>(url, {
      method: "GET",
    });
    const data = response?.data ?? response;
    return {
      plotId: String(data.plotId ?? id),
  ...
  async generateBulkQRCodes(
    ...
    const url = `${this.baseUrl}/plots/bulk-qr-generation`;
    const payload = { plotIds };
    const response: any = await this.fetchWithErrorHandling<any>(url, {
      method: "POST",
      body: JSON.stringify(payload),
    });
    const data = response?.data ?? response;
    const list: Array<{ id: string; qrCode: string }> = Array.isArray(data)
      ? data
    ...
    return list.map((x) => ({
      id: String(x.id ?? ""),
      qrCode: String(x.qrCode ?? ""),
    }));
  }

  async generatePlotQRCode(id: string): Promise<string> {
    const list = await this.generateBulkQRCodes([id]);
    const entry = list[0];
    if (!entry?.qrCode) {
      throw new Error("Falha ao gerar QR Code para a sepultura");
    }
    return entry.qrCode;
  }
