...
import type { Plot, PlotFormData } from "@/app/(myapp)/types/Plot";
...
export default function PlotsPage() {
  ...
  const { plots, fetchPlots, createPlot, updatePlot, isLoading } = usePlot();
  ...
  const goToEditPlot = (plotId: string) => {
    if (!selectedCemeteryId) return;
    router.push(`/cemeteries/${selectedCemeteryId}/plots/${plotId}/edit`);
  };
   ...
   */
  const doCancelReservation = async (_p: Plot) => {
    igrpToast({
      title: "Erro",
   ...
   */
  const doAvailable = async (_p: Plot) => {
    igrpToast({
      title: "Erro",
                  ...
                  cell: ({ row }) => {
                    ...
                    return (
                      <IGRPDataTableRowAction>
                        <IGRPDataTableButtonLink
                          labelTrigger={`Editar`}
                          href={`/cemeteries/${selectedCemeteryId}/plots/${p.id}/edit`}
                          variant={`ghost`}
                          icon={`Pencil`}
                        ></IGRPDataTableButtonLink>
                        <IGRPDataTableDropdownMenu
