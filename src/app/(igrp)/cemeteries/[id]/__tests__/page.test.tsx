...
describe("CemeteryDetailPage", () => {
  ...
  it("renders basic info correctly", () => {
    ...
    expect(screen.getByText("Test Cemetery")).toBeInTheDocument();
    expect(screen.getByText("Test Address")).toBeInTheDocument();
    expect(screen.getByText(/1000/)).toBeInTheDocument(); // Capacity
    expect(screen.getByText(/5000/)).toBeInTheDocument(); // Area

    // Check Blocks
    const statsGrid = screen.getByTestId("cemetery-basic-stats");
    const blocksLabel = within(statsGrid).getByText("Blocos Disponíveis");
    expect(blocksLabel).toBeInTheDocument();
    expect(blocksLabel.nextSibling).toHaveTextContent("1");

    // Check Sections
    const sectionsLabel = within(statsGrid).getByText("Total de Seções");
    expect(sectionsLabel).toBeInTheDocument();
    expect(sectionsLabel.nextSibling).toHaveTextContent("1");
  });

  it('renders "Ver no mapa" button and handles click', () => {
    render(<CemeteryDetailPage />);
    ...
    expect(mockFlyTo).toHaveBeenCalledWith(
      expect.objectContaining({
        center: [20, 10], // Longitude, Latitude
        zoom: 16,
      }),
    );
  });
